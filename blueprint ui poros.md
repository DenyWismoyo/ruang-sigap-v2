# Poros UI Premium Rebrand — Rencana Implementasi

## Latar Belakang

Saat ini Poros menggunakan sistem CSS yang masih **terlalu mirip** dengan Sigap — keduanya berbagi pola komponen yang sama, spacing yang hampir identik, dan pendekatan visual "bersih tapi generik". Tujuan refaktor ini adalah menjadikan Poros terasa **corporate-luxury**, elegan, dan punya identitas visual yang kuat secara independen, tanpa mengubah satu pun alur UX.

---

## Audit: Kondisi Saat Ini

### Sigap vs Poros — Perbedaan yang ada:
| Aspek | SIGAP | POROS (sekarang) |
|---|---|---|
...
### Kelemahan yang Ditemukan di Poros:
...
2. **`poros.css` terlalu tipis (160 baris)** — belum ada token desain untuk card premium, gradient accent, dll.
...
5. **MegaMenuPanel** dan **Sidebar** sudah baik, tapi belum punya "signature visual" Poros yang kuat.
6. **Header (layout.tsx)** — bell, search, avatar button masih sangat generic/polos.
7. **Page transitions** sudah ada tapi loading overlay belum memakai identitas Poros.
8. **Halaman-halaman turunan** (surat, tugas, dsb) hampir tidak punya sentuhan brand Poros — styling sepenuhnya bergantung pada Shadcn defaults.

---

## Filosofi Desain Poros Premium
...
**File: [`poros.css`](file:///d:/DENY/project/ruang-sigap-v2/src/app/dashboard/poros/poros.css)**
...
**Dampak**: Seluruh tampilan Poros langsung berubah secara pasif karena CSS variables ter-inherit.

---

### FASE 2 — Shell: Layout, Header & Navigation

#### [MODIFY] [`layout.tsx`](file:///d:/DENY/project/ruang-sigap-v2/src/app/dashboard/poros/layout.tsx)
...
#### [MODIFY] [`Sidebar.tsx`](file:///d:/DENY/project/ruang-sigap-v2/src/app/dashboard/poros/components/Sidebar.tsx)
...
#### [MODIFY] [`MegaMenuPanel.tsx`](file:///d:/DENY/project/ruang-sigap-v2/src/app/dashboard/poros/components/MegaMenuPanel.tsx)
...
#### [MODIFY] [`BottomNavBar.tsx`](file:///d:/DENY/project/ruang-sigap-v2/src/app/dashboard/poros/components/BottomNavBar.tsx)
...
#### [MODIFY] [`page.tsx`](file:///d:/DENY/project/ruang-sigap-v2/src/app/dashboard/poros/page.tsx)
...
#### [MODIFY] [`SmartGreeting.tsx`](file:///d:/DENY/project/ruang-sigap-v2/src/app/dashboard/poros/components/home/SmartGreeting.tsx)
...
#### [MODIFY] [`QuickAccessCard.tsx`](file:///d:/DENY/project/ruang-sigap-v2/src/app/dashboard/poros/components/home/QuickAccessCard.tsx)
...
> **Strategi**: Buat utility CSS classes di `poros.css` yang cukup deklaratif, lalu update komponen-komponen berulang di setiap halaman secara batch.
...
|---|---|---|
| Kotak Masuk Surat | [`(main)/surat/page.tsx`](file:///d:/DENY/project/ruang-sigap-v2/src/app/dashboard/poros/(main)/surat/page.tsx) | Tabel surat, badge status, tab filter |
| Tugas Saya | [`(main)/tugas/page.tsx`](file:///d:/DENY/project/ruang-sigap-v2/src/app/dashboard/poros/(main)/tugas) | Card tugas, progress bar, badge prioritas |
| Logbook Harian | [`(main)/logbook/page.tsx`](file:///d:/DENY/project/ruang-sigap-v2/src/app/dashboard/poros/(main)/logbook) | Card logbook, timeline entry |
| Ruang Kerja | [`(main)/ruang-kerja/page.tsx`](file:///d:/DENY/project/ruang-sigap-v2/src/app/dashboard/poros/(main)/ruang-kerja) | Widget statistik, panel kerja |
...
3. **Badge status** → override warna dengan token poros, bukan Tailwind random
...
### Manual Verification:
- Buka `localhost:3000/dashboard` (poros tenant)
- Cek light & dark mode
...
2. **Gold accent**: Apakah Anda setuju dengan penambahan **gold/khaki accent** sebagai warna secondary yang membedakan Poros? Ini yang paling membedakan dari Sigap yang monokrom.

3. **Editorial left-line**: Apakah konsep "setiap card punya garis tipis teal di kiri" disetujui? Ini signature Poros yang akan sangat konsisten.
