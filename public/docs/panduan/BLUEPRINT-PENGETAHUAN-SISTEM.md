...
**Sistem ini** adalah platform manajemen persuratan digital terpadu yang dirancang khusus untuk **Instansi** di lingkungan Pemerintah Daerah.

Sistem ini bukan sekadar aplikasi input-output surat biasa. Sistem ini adalah **ekosistem administrasi digital terintegrasi** yang menghubungkan seluruh rantai birokrasi persuratan — dari Staf TU yang menginput surat, Pimpinan yang mendisposisikan, Pelaksana yang menindaklanjuti, hingga dokumen arsip yang tersimpan secara permanen.
...
## 1.3 Masalah yang Dipecahkan Sistem

| Masalah Lama (Manual)                                | Solusi Sistem                                             |
| ---------------------------------------------------- | --------------------------------------------------------- |
...
Sistem mengenal 3 jenis pengguna utama dengan hak akses yang berbeda:
...
- Dapat melihat seluruh surat Instansi
...
- Melihat laporan kinerja tim/Instansi
...
Sistem menggunakan sistem level jabatan untuk menentukan hierarki disposisi:
...
Surat masuk baru (yang belum ada tujuan jabatan spesifik) akan muncul di Ruang Kerja pimpinan teratas Instansi (level terendah angkanya = posisi tertinggi).
...
Ini adalah pusat manajemen seluruh surat masuk Instansi.
...
**Path:** `/dashboard/laporan` (berbeda dengan halaman laporan Instansi)
...
Statistik dan analitika kinerja Instansi secara keseluruhan.
...
Sistem menghasilkan laporan mingguan otomatis yang tersimpan di database dan dapat diakses Super Admin untuk perbandingan antar Instansi.
...
Dapat dibagikan ke Instansi lain (fitur paket Pro/Enterprise).
...
Dapat dibagikan ke Instansi lain.
...
Repositori artikel pengetahuan, SOP, panduan internal Instansi.
Dapat dikategorikan dan dibagikan ke Instansi lain.
...
Manajemen surat keluar Instansi terpisah dari surat masuk.
Pencatatan surat yang dikirim keluar oleh Instansi.
...
## 7.4 Dua Tema UI (Sistem & Sistem)

Sistem memiliki dua varian tampilan:

- **Sistem** — Tema modern, clean, professional (default/utama)
- **Sistem** — Tema lama (sedang dalam fase transisi)

Admin dapat mengatur tema default Instansi. User dapat memilih tema individual di Profil.
...
| Koleksi         | Isi                                          |
| --------------- | -------------------------------------------- |
| `surat`         | Data semua surat masuk Instansi              |
...
## 9.2 Isolasi Data Antar Instansi

Setiap data di-tag dengan `opdId`. Pengguna hanya bisa melihat data Instansi-nya sendiri. Tidak ada kebocoran data lintas Instansi kecuali melalui fitur Surat Lintas Instansi yang eksplisit.
...
**Q: Bisa tidak mendisposisikan surat ke seseorang di luar Instansi saya?**
A: Ya, melalui fitur Surat Lintas Instansi (untuk paket Enterprise). Pastikan Instansi tujuan sudah terdaftar dalam sistem.
...
A: Bergantung pada paket langganan Instansi (Dasar/Profesional/Enterprise). Lihat detail di pengaturan Instansi.
...
Masyarakat mengajukan permohonan online → otomatis masuk sebagai surat masuk di Sistem → status dapat dilacak masyarakat secara real-time

**Dashboard Eksekutif:**
Kepala Daerah (Bupati/Walikota) dapat melihat kinerja seluruh Instansi dalam satu tampilan konsolidasi
...
_Dokumen ini adalah pengetahuan lengkap sistem Sistem versi produksi Agustus 2026._
_Dihasilkan dari audit mendalam kode sumber: 29 custom hooks, 30+ halaman, seluruh alur bisnis._
