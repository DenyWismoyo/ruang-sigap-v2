...
interface DemoGuideCopilotProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function DemoGuideCopilot({ isOpen, onClose }: DemoGuideCopilotProps) {
  ...
  useEffect(() => {
    ...
    if (isOpen && chapters.length === 0) {
      const loadGuide = async () => {
        try {
          setIsLoading(true);
          const response = await fetch('/docs/panduan/PANDUAN_DEMO.md');
            ...
            >
              <div className="flex-1 pr-4 pointer-events-none">
                  <div className="flex items-center gap-2 text-foreground">
                      ...
                      <h3 className="font-bold">Panduan Penggunaan Demo</h3>
                  </div>
              ...
              <div className="absolute top-4 right-4 flex gap-1 z-10">
                  ...
                  <Button variant="ghost" size="icon" onClick={onClose} title="Tutup Panduan" className="h-8 w-8 text-muted-foreground hover:text-destructive">
                    <X size={16} />
                  </Button>
              </div>
            </div>
