/**
 * Directory: src/app/dashboard/sigap/(main)/tugas/components/FormTugas.tsx
 * Status: REFACTORED - DIRECT DIRECTIVE & MODERN UX
 * Deskripsi: Form Pembuatan Tugas Modern (Instruksi Tim, Multi-Assignee, Voice Memo, Preset Deadline).
 */
...
export default function FormTugas({ isOpen, onClose, onSuccess, userCache, suratTerkait, initialData }: FormTugasProps) {
  ...
  // Mode: 'delegasi' (Instruksi Tim) vs 'mandiri' (Catatan Pribadi)
  ...
  const setQuickDeadline = (type: 'today' | 'tomorrow' | '3days' | 'weekend') => {
    ...
    if (type === 'today') {
      ...
      const day = d.getDay();
      const diff = (day === 0 ? 0 : 5 - day); // Target hari Jumat
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3. Mode Delegasi Tim: Split Mode vs Kolaborasi Mode
      if (assignmentMode === 'split' && selectedStaffList.length > 1) {
        // Buat batch tiket personal
        ...
        // Mode Kolaborasi: Staf pertama = PJ Utama, sisanya = Kolaborator
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full h-[95vh] sm:h-auto sm:max-h-[92vh] sm:max-w-2xl bg-card border-border flex flex-col p-0 gap-0 shadow-2xl">
        
        {/* Header Dialog */}
        <DialogHeader className="p-4 md:p-6 pb-3 border-b border-border bg-muted/20">
          ...
          {/* Mode Switcher */}
          {!suratTerkait && (
            <div className="grid grid-cols-2 gap-2 mt-4 p-1 bg-muted/60 rounded-xl border border-border">
              <button
                type="button"
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
                  ...
                  <div className="flex flex-wrap gap-1.5 min-h-[36px] items-center">
                    {selectedStaffList.map((staff, idx) => (
                      <Badge
                        key={staff.uid}
                        variant="secondary"
                        className="pl-2 pr-1 py-1 gap-1 text-xs bg-blue-50 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-blue-800"
                      >
                  ...
                  {linkedSurat ? (
                    <div className="flex items-center justify-between p-2.5 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                      <div className="truncate pr-2">
                        <p className="text-xs font-semibold text-blue-900 dark:text-blue-300 truncate">{linkedSurat.perihal}</p>
                        <p className="text-[11px] text-blue-700/80 dark:text-blue-400 truncate">{linkedSurat.nomorSurat}</p>
                      </div>
                      <Button
                        type="button"
                        variant="ghost"
          ...
          <div className="p-4 border-t border-border bg-muted/20 flex items-center justify-end gap-2">
            ...
            <Button
              ...
              className="text-xs h-9 px-5 font-semibold gap-1.5 bg-blue-600 hover:bg-blue-700 text-white"
            >
              {loading ? (
                <>
