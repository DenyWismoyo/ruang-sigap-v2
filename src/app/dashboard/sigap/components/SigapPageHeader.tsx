  ...
  return (
    <div className="sg-page-header">
      <div className="flex-1 min-w-0">
        ...
        {description && <p className="text-xs md:text-sm text-muted-foreground mt-0.5 md:mt-1 line-clamp-1 md:line-clamp-none">{description}</p>}
        {children && <div className="mt-3 md:mt-4">{children}</div>}
      </div>
      {actions && <div className="sg-page-actions shrink-0">{actions}</div>}
    </div>
  );
}
