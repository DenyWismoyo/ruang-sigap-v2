...
const StatCard = ({ title, value, icon, colorClass }: { title: string; value: string | number; icon: React.ReactNode; colorClass: string }) => (
    <div className="nk-card p-6">
      <div className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <div className={`p-3 rounded-full ${colorClass.replace('text-', 'bg-').replace('600', '100')} bg-opacity-20`}>
...
export default function LaporanPage() {
    ...
    return (
        <div>
            ...
            <div className="nk-card p-4 mt-6">
                <div className="mb-2">
                    <Label htmlFor="reportDate" className="font-bold text-foreground">Tampilkan Laporan Mingguan Terbaru Dari Tanggal:</Label>
                </div>
                <Input id="reportDate" type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)} className="w-auto bg-transparent border-white/10 dark:border-white/5"/>
            </div>
            ...
            {loading ? <p className='text-center text-muted-foreground p-8'>Memuat laporan...</p> : (
                <>
                    ...
                    <div className="nk-card overflow-hidden mt-8">
                        <div className="p-6 border-b border-white/10 dark:border-white/5">
                            <h2 className="text-xl font-semibold text-foreground">Surat per OPD (Mingguan)</h2>
                        </div>
                        <div className="p-6"><canvas ref={chartRef}></canvas></div>
                    </div>
                    <div className="nk-card overflow-hidden mt-8">
                        <div className="p-6 border-b border-white/10 dark:border-white/5">
                            <h2 className="text-xl font-semibold text-foreground">Detail Laporan per OPD</h2>
                        </div>
                        <div>
                            <div className="nk-table-wrapper overflow-x-auto">
                                <Table>
                                    <TableHeader className="border-b border-white/10 dark:border-white/5 bg-background/50 backdrop-blur-md">
