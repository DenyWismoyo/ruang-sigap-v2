...
export default function RepositoryDokumenPage() {
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col md:flex-row gap-3 md:gap-4 justify-between items-start md:items-center p-4 md:p-6 bg-card border-b border-[var(--nk-gradient-start)]/10 shadow-sm backdrop-blur-xl">
                <div className="flex items-center gap-3">
                    <div className="bg-[var(--nk-teal-mid)]/10 p-2.5 rounded-xl text-[var(--nk-teal-mid)]">
                        <FolderArchive className="w-6 h-6" />
                    </div>
                    <div>
                        <h1 className="text-xl md:text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[var(--nk-gradient-start)] to-[var(--nk-gradient-end)]">Repository Dokumen</h1>
                        <p className="text-sm text-muted-foreground hidden md:block">Pusat penyimpanan dan pengelolaan dokumen, file, dan tautan penting OPD.</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-auto p-3 md:p-6 pb-[calc(4.5rem+env(safe-area-bottom,0px))] md:pb-6 space-y-4 md:space-y-6">
                ...
                <div className="flex flex-col gap-4">
                    <Tabs value={activeZone} onValueChange={(val) => { setActiveZone(val as any); setCurrentFolderId(null); setFolderPath([]); }}>
                        <TabsList className="grid w-full grid-cols-4 md:max-w-xl h-auto p-1 bg-white/40 dark:bg-black/40 rounded-xl backdrop-blur-md">
                            <TabsTrigger value="opd" className="py-2 rounded-lg data-[state=active]:bg-white/80 dark:data-[state=active]:bg-gray-800/80 data-[state=active]:text-[var(--nk-teal-mid)] dark:data-[state=active]:text-white font-medium text-xs md:text-sm transition-all shadow-sm">Arsip OPD</TabsTrigger>
                            <TabsTrigger value="private" className="py-2 rounded-lg data-[state=active]:bg-white/80 dark:data-[state=active]:bg-gray-800/80 data-[state=active]:text-[var(--nk-teal-mid)] dark:data-[state=active]:text-white font-medium text-xs md:text-sm transition-all shadow-sm">Pribadi</TabsTrigger>
                            <TabsTrigger value="shared" className="py-2 rounded-lg data-[state=active]:bg-white/80 dark:data-[state=active]:bg-gray-800/80 data-[state=active]:text-[var(--nk-teal-mid)] dark:data-[state=active]:text-white font-medium text-xs md:text-sm transition-all shadow-sm">Dibagikan</TabsTrigger>
                            <TabsTrigger value="trash" className="py-2 rounded-lg data-[state=active]:bg-red-500/10 data-[state=active]:text-red-600 dark:data-[state=active]:text-red-400 font-medium text-xs md:text-sm transition-all shadow-sm">Sampah</TabsTrigger>
                        </TabsList>
                    </Tabs>
                    
                    {activeZone === 'trash' && (
                        <div className="bg-red-50 text-red-800 p-3 rounded-xl text-sm flex items-center gap-2 border border-red-100 shadow-sm">
                ...
                <div className="flex flex-col md:flex-row gap-3 md:gap-4 justify-between items-center bg-card p-3 md:p-4 sg-mobile-borderless">
                    ...
                    <div className="flex w-full md:w-auto items-center gap-2 justify-between md:justify-end">
                        <div className="flex items-center gap-1 bg-white/50 dark:bg-black/50 p-1 rounded-lg backdrop-blur-sm border border-white/20">
                            <button onClick={() => setViewMode('list')} className={`p-1.5 rounded-md transition-colors ${viewMode === 'list' ? 'bg-white dark:bg-gray-800 shadow-sm text-[var(--nk-teal-dark)] dark:text-white' : 'text-muted-foreground'}`}><ListIcon size={16}/></button>
                            <button onClick={() => setViewMode('grid')} className={`p-1.5 rounded-md transition-colors ${viewMode === 'grid' ? 'bg-white dark:bg-gray-800 shadow-sm text-[var(--nk-teal-dark)] dark:text-white' : 'text-muted-foreground'}`}><LayoutGrid size={16}/></button>
                        </div>
                        
                        {repo.canCreate && activeZone !== 'trash' && (
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <Button className="bg-gradient-to-r from-[var(--nk-teal-mid)] to-[var(--nk-teal-dark)] text-white shadow-md hover:shadow-lg border-0 rounded-xl transition-all"><Plus className="w-4 h-4 mr-2" /> Tambah Baru</Button>
                                </DropdownMenuTrigger>
                ...
                {repo.loading ? (
                    ...
                    <SigapEmptyState 
                        ...
                        action={repo.canCreate && !searchTerm && activeZone !== 'trash' ? (
                            <Button className="bg-gradient-to-r from-[var(--nk-teal-mid)] to-[var(--nk-teal-dark)] text-white shadow-md hover:shadow-lg border-0 rounded-xl transition-all" onClick={() => setModalState({ isOpen: true, mode: 'folder', item: null })}>
                                Buat Folder Pertama
                            </Button>
                        ) : undefined}
                    />
                        ...
                        {currentItems.map((item) => (
                            ...
                            >
                                <div className={`absolute top-2 left-2 z-10 transition-opacity ${selectedItemIds.includes(item.id!) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                                    <div 
                                        className={`w-5 h-5 rounded border flex items-center justify-center cursor-pointer transition-colors ${selectedItemIds.includes(item.id!) ? 'bg-[var(--nk-teal-mid)] border-[var(--nk-teal-mid)]' : 'bg-background/80 border-muted-foreground hover:border-[var(--nk-teal-mid)]'}`}
                                ...
                                >
                                    ...
                                    <div className={`flex-1 min-w-0 ${viewMode === 'grid' ? 'w-full px-2' : ''}`}>
                                        <div className="flex items-center gap-2 mb-1 w-full overflow-hidden">
                                            <p className={`font-semibold text-sm ${item.tipe !== 'folder' ? 'text-[var(--nk-teal-mid)]' : 'text-foreground'} truncate`} title={item.nama}>
                                                {item.nama}
                                            </p>
                                            {item.aiClassification && (
                                                <span className="hidden md:inline-flex shrink-0 px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-600 border border-amber-500/20 text-[10px] font-semibold tracking-wide" title={item.aiSummary}>
                                                    ✨ {item.aiClassification}
                                                </span>
                                            )}
                                        </div>
                                ...
                                <div className={`${viewMode === 'grid' ? 'absolute top-2 right-2' : ''}`}>
                                    {repo.canManageItem(item) ? (
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="bg-white/95 dark:bg-black/95 backdrop-blur-xl border border-white/20 shadow-xl rounded-xl">
                                                {activeZone === 'trash' ? (
                                                    <>
                                                        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); repo.restoreItem(item.id!); }} className="rounded-lg hover:bg-[var(--nk-teal-mid)]/10">
                                                            Pulihkan Item
                                                        </DropdownMenuItem>
                                                        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); repo.hardDeleteItem(item); }} className="text-red-600 focus:bg-red-50 dark:focus:bg-red-950 rounded-lg">
                                                            <Trash2 className="mr-2 h-4 w-4" /> Hapus Permanen
                                                        </DropdownMenuItem>
                                                    </>
                                                ) : (
                                                    <>
                                                        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); repo.toggleFavorite(item.id!, !!item.isFavorite); }} className="rounded-lg hover:bg-[var(--nk-teal-mid)]/10">
                                                        ...
                                                        <DropdownMenuItem onClick={async (e) => {
                                                            const url = `${window.location.origin}/dashboard/poros/dokumen?itemId=${item.id}`;
                                                        ...
                                                        }} className="rounded-lg hover:bg-[var(--nk-teal-mid)]/10">
                                                            <LinkIcon className="w-4 h-4 mr-2" /> Salin Tautan
                                                        </DropdownMenuItem>
                                                        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); setModalState({ isOpen: true, mode: item.tipe as any, item }); }} className="rounded-lg hover:bg-[var(--nk-teal-mid)]/10">
                                                            <Edit className="mr-2 h-4 w-4" /> Edit
                                                        </DropdownMenuItem>
                                                        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleDelete(item); }} className="text-red-600 focus:bg-red-50 dark:focus:bg-red-950 rounded-lg">
                                        ...
                                        item.tipe !== 'folder' && item.url && (
                                            <Button variant="ghost" size="icon" className="h-8 w-8 opacity-50 group-hover:opacity-100 hover:bg-[var(--nk-teal-mid)]/10 rounded-full" onClick={async () => {
                                                if (userProfile) {
