    ...
    return (
        <div className="pb-[calc(5.5rem+env(safe-area-inset-bottom,0px))] md:pb-6">
            ...
            <div className="block md:hidden mb-4">
                ...
                {mobileTab === 'agenda' && (
                    <div className="mt-3 space-y-2 md:space-y-3">
                        {/* Month Selector Bar */}
                        <div className="flex items-center justify-between px-3 py-2 bg-card nk-mobile-borderless shadow-none md:shadow-sm">
                        ...
                        {!loading && agendaBulanIni.length === 0 && (
                            <div className="p-8 text-center bg-card nk-mobile-borderless border border-dashed border-border/60">
                        ...
                        {!loading && agendaBulanIni.length > 0 && agendaBulanIni.map(jadwal => {
                            ...
                            return (
                                <div 
                                    key={jadwal.id} 
                                    onClick={() => handleOpenDetailModal(jadwal)}
                                    className="p-3.5 bg-card nk-mobile-borderless border-b border-border/40 active:scale-[0.99] transition-all cursor-pointer"
                                >
            ...
            <JadwalDetailModal
                ...
                isAdmin={isAdmin}
                onApprove={(id) => handleApprove(id, () => { setIsDetailModalOpen(false); fetchData(); })}
                onReject={(id, reason) => handleReject(id, reason, () => { setIsDetailModalOpen(false); fetchData(); })}
                onEdit={(j) => { setIsDetailModalOpen(false); handleOpenFormModal(j.tanggalMulai.toDate(), j); }}
                onDelete={(id) => handleDelete(id, () => { setIsDetailModalOpen(false); fetchData(); })}
            />
        </div>
    );
}
