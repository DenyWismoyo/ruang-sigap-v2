...
export default function JadwalDetailModal({ isOpen, onClose, jadwal, isAdmin, onApprove, onReject, onEdit, onDelete }: JadwalDetailModalProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={handleModalClose}>
            <DialogContent className="max-w-[95vw] sm:max-w-lg w-full max-h-[90vh] overflow-y-auto p-5 sm:p-6 bg-card/95 backdrop-blur-xl border border-[var(--nk-gradient-start)]/20 shadow-[0_8px_40px_-10px_rgba(17,94,89,0.3)] rounded-2xl">
                            ...
                            <div className="pt-2">
                                ...
                                <div className="max-h-48 overflow-y-auto rounded-xl border border-border/70 bg-muted/20 divide-y divide-border/40 shadow-inner">
                                    {jadwal.peserta.map((p, idx) => (
                                        ...
                                        >
                                            <div className="w-5 h-5 rounded-full bg-teal-500/15 text-teal-600 dark:text-teal-400 text-[10px] font-bold flex items-center justify-center shrink-0">
                                                {idx + 1}
                                            </div>
                            ...
                            <div className="pt-3 border-t border-border/40 mt-3 w-full min-w-0">
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 p-3 rounded-xl bg-muted/30 border border-border/60 w-full min-w-0">
                                    <div className="flex items-center gap-2.5 min-w-0 flex-1">
                                        <div className="w-8 h-8 rounded-lg bg-teal-100 dark:bg-teal-900/40 text-teal-600 dark:text-teal-300 flex items-center justify-center shrink-0">
                                            <FileText size={16} />
                                        </div>
                                    ...
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="inline-flex items-center justify-center gap-1.5 text-xs font-medium text-teal-600 hover:text-teal-700 dark:text-teal-400 dark:hover:text-teal-300 bg-teal-50 dark:bg-teal-950/50 hover:bg-teal-100 dark:hover:bg-teal-900/50 px-3 py-1.5 rounded-lg transition-colors shrink-0"
                                    >
