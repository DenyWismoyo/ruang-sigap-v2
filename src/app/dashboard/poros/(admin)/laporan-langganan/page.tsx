...
// - [FITUR BARU] Menambahkan Tombol & Logika Auto Heal Database.
    ...
    return (
        <div className="nk-card p-6">
          <div className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <div className={`p-2 rounded-full ${colorClass.replace('text-', 'bg-').replace('600', '100')} bg-opacity-20`}>
                ...
                <form onSubmit={onSave}>
                    <ScrollArea className="max-h-[70vh]">
                        <div className="px-6 space-y-4">
                            ...
                            <div>
                                <Label htmlFor="pkg-harga">Harga Paket per Bulan (Rp)</Label>
                                <Input 
                                    id="pkg-harga"
                                    type="number" 
                                    value={packageData.hargaBulanan || 0} 
                                    onChange={e => setPackageData({ ...packageData, hargaBulanan: Number(e.target.value) })} 
                                />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <Label htmlFor="pkg-batas-pengguna">Batas Maksimal Pengguna (-1 untuk Tak Terbatas)</Label>
                                    <Input 
                                        id="pkg-batas-pengguna"
                                        type="number" 
                                        value={packageData.batasPengguna !== undefined ? packageData.batasPengguna : 0} 
                                        onChange={e => setPackageData({ ...packageData, batasPengguna: Number(e.target.value) })} 
                                    />
                                </div>
                                <div>
                                    <Label htmlFor="pkg-batas-surat">Batas Surat / Bulan (-1 untuk Tak Terbatas)</Label>
                                    <Input 
                                        id="pkg-batas-surat"
                                        type="number" 
                                        value={packageData.batasSuratBulanan !== undefined ? packageData.batasSuratBulanan : 0} 
                                        onChange={e => setPackageData({ ...packageData, batasSuratBulanan: Number(e.target.value) })} 
                                    />
                                </div>
                            </div>
                            <div className="space-y-2">
...
const LaporanLanggananPage = () => {
    ...
    const [proyeksiList, setProyeksiList] = useState<Record<string, { suratBulanIni?: number, recommendedPackage?: string, hargaBulanan?: number, loading: boolean, error?: boolean }>>({});
    ...
    useEffect(() => {
        let isMounted = true;
        const fetchAllProyeksi = async () => {
            if (opdList.length === 0 || pricingPackages.length === 0) return;
            const now = new Date();
            const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
            
            const initialLoadingState: any = {};
            opdList.forEach(opd => { initialLoadingState[opd.id!] = { loading: true }; });
            if (isMounted) setProyeksiList(initialLoadingState);
            
            const promises = opdList.map(async (opd) => {
                try {
                    ...
                    return {
                        ...
                        data: {
                            loading: false,
                            suratBulanIni,
                            recommendedPackage: recommendedPackage?.id || 'N/A',
                            hargaBulanan: recommendedPackage?.hargaBulanan || 0
                        }
                    };
                } catch (error) {
                    return { id: opd.id!, data: { loading: false, error: true } };
                }
            });
            ...
            const newProyeksiList: any = {};
            results.forEach(res => { newProyeksiList[res.id] = res.data; });
            setProyeksiList(newProyeksiList);
        };
        fetchAllProyeksi();
        return () => { isMounted = false; };
    }, [opdList, pricingPackages, opdConfigs]);
    ...
    const openPackageModal = (pkg: PricingPackage | null) => {
        if (pkg) {
            ...
            setPackageFormState({ id: '', hargaBulanan: 0, batasPengguna: -1, batasSuratBulanan: -1, features: defaultFeatures });
    ...
    // --- TAMBAHAN FUNGSI AUTO HEAL ---
    const handleRunAutoHeal = () => {
        setConfirmModal({
            isOpen: true,
            title: 'Jalankan Auto Heal?',
            message: 'Sistem akan memindai seluruh database untuk mencari dan memperbaiki inkonsistensi data (seperti counter notifikasi minus atau error cache). Proses ini memakan waktu beberapa detik. Lanjutkan?',
            isProcessing: false,
            onConfirm: async () => {
                ...
                try {
                    ...
                    alert(`Gagal menjalankan Auto Heal: ${error.message}`);
                } finally {
                    setConfirmModal({ isOpen: false, title: '', message: '', onConfirm: () => {}, isProcessing: false });
                }
            }
    ...
    return (
        <div className="animate-fadeInUp">
            {/* [PERBAIKAN] Flex container untuk menjejajarkan Judul dan Tombol Auto Heal */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <Button onClick={handleRunAutoHeal} className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm">
                    <Stethoscope className="mr-2 h-4 w-4" /> Diagnosis & Auto Heal
                </Button>
            </div>
            ...
            <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="mt-8">
                ...
                <TabsContent value="proyeksi" className="mt-6 animate-fadeInUp">
                    <div className="nk-card overflow-hidden">
                        <div className="p-4 border-b border-white/10 dark:border-white/5 flex justify-between items-center"><h2 className="text-xl font-semibold text-foreground">Proyeksi & Status Langganan OPD</h2></div>
                        <div className="p-4">
                        ...
                        {filteredOpd.length > 0 && (
                            <div className="overflow-x-auto">
                                <Table>
                                    <TableHeader>
                                        <TableRow><TableHead>Nama OPD</TableHead><TableHead>Paket</TableHead><TableHead>Aktif Hingga</TableHead><TableHead>Jml Pengguna</TableHead><TableHead>Jml Surat</TableHead><TableHead>Proyeksi Rp</TableHead><TableHead>Status</TableHead><TableHead>Aksi</TableHead></TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {filteredOpd.map(opd => {
                                            const config = opdConfigs.get(opd.id!);
                                            ...
                                            return (
                                                <TableRow key={opd.id}>
                                                    ...
                                                    <TableCell className="font-medium text-center">
                                                        {proyeksiList[opd.id!]?.loading ? <span className="text-gray-400">...</span> : proyeksiList[opd.id!]?.suratBulanIni?.toLocaleString('id-ID') || 0}
                                                    </TableCell>
                                                    <TableCell className="font-bold text-blue-600">
                                                        {proyeksiList[opd.id!]?.loading ? <span className="text-gray-400">...</span> : `Rp ${(proyeksiList[opd.id!]?.hargaBulanan || 0).toLocaleString('id-ID')}`}
                                                    </TableCell>
                                                    <TableCell>
                ...
                {/* --- TAB 2: TAGIHAN --- */}
                <TabsContent value="tagihan" className="mt-6 animate-fadeInUp">
                     <div className="nk-card overflow-hidden">
                        <div className="p-4 border-b border-white/10 dark:border-white/5 flex flex-col md:flex-row justify-between md:items-center gap-4">
                            <h2 className="text-xl font-semibold text-foreground">Daftar Tagihan</h2>
                        ...
                        <div className="nk-table-wrapper overflow-x-auto">
                            <Table>
                                <TableHeader>
                ...
                <TabsContent value="paket" className="mt-6 animate-fadeInUp">
                    <div className="nk-card overflow-hidden">
                        ...
                        <div className="nk-table-wrapper p-4 overflow-x-auto">
                            <Table>
                                <TableHeader>
