...
import { collection, query, where, addDoc, doc, updateDoc, deleteDoc, Timestamp, orderBy, getDocs, or, and, arrayUnion } from 'firebase/firestore';
...
import { 
  ...
  Clock, ChevronDown, ChevronUp, Eye, Sparkles
} from 'lucide-react';
...
const HeroAnnouncement = ({ 
    ...
    p: Pengumuman, isAdmin: boolean, setLightboxData: any, onEdit: any, onDelete: any, userProfile: any, onMarkRead: (id: string, readBy?: string[]) => void
}) => {
        ...
        <div className="mb-8 group relative nk-card rounded-2xl border border-[var(--border)] shadow-[var(--nk-shadow-lg)] overflow-hidden hover:shadow-xl transition-all duration-300">
            ...
            <div className={`flex flex-col ${hasImage ? 'lg:flex-row' : ''} h-full`}>
                ...
                <div className={`${hasImage ? 'lg:w-5/12' : 'w-full'} flex flex-col p-6 lg:p-8 justify-center relative`}>
                     ...
                     {/* Metadata */}
                     <div className="flex items-center flex-wrap gap-2 text-xs font-medium text-muted-foreground mb-3">
                        <span className="flex items-center gap-1 bg-muted px-2 py-1 rounded-md">
                           <Clock size={12} /> {formatDateRelative(p.createdAt)}
                        </span>
                        <span className="flex items-center gap-1 bg-muted px-2 py-1 rounded-md" title="Jumlah Tayangan">
                           <Eye size={12} /> {p.readBy?.length || 0}
                        </span>
                        {p.target === 'Semua OPD' && <Badge variant="secondary" className="text-[10px]">Global</Badge>}
                     </div>

                     {/* Judul Besar */}
                     <h2 className="text-2xl lg:text-3xl font-extrabold text-foreground leading-tight mb-4 group-hover:text-primary transition-colors cursor-pointer" onClick={handleExpand}>
                        {p.judul}
                     </h2>

                     {/* Isi Excerpt (Expandable) */}
                     <div 
                        className={`prose prose-sm max-w-none dark:prose-invert text-foreground/80 mb-4 cursor-pointer ${isExpanded ? '' : 'line-clamp-4 lg:line-clamp-6'}`}
                        onClick={handleExpand}
                     >
                        <ReactMarkdown remarkPlugins={[remarkGfm]}>{p.isi}</ReactMarkdown>
                     </div>
            ...
            <CardContent className="flex-1 p-5 flex flex-col">
                ...
                <div className="flex justify-between items-center mb-2 text-xs text-muted-foreground">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="flex items-center gap-0.5 text-muted-foreground bg-muted/50 px-1.5 py-0.5 rounded-sm" title="Jumlah Tayangan">
                           <Eye size={10} /> {p.readBy?.length || 0}
                        </span>
                    </div>
                ...
                <div 
                    className={`prose prose-sm max-w-none dark:prose-invert text-muted-foreground text-sm mb-2 flex-1 cursor-pointer ${isExpanded ? '' : 'line-clamp-3'}`}
                    onClick={handleExpand}
                >
                     <ReactMarkdown remarkPlugins={[remarkGfm]}>{p.isi}</ReactMarkdown>
                </div>
    ...
    const handleGenerateAI = async () => {
        ...
        try {
            ...
            if (data.content) {
                setFormData(prev => {
                    if (!prev) return prev;
                    const newIsi = prev.isi ? `${prev.isi}\n\n${data.content}` : data.content;
                    return { ...prev, isi: newIsi };
                });
                setAiPrompt('');
                addToast("Teks berhasil dirumuskan oleh AI.", "success");
            } else {
                addToast(data.error || "Gagal generate dengan AI.", "error");
            }
        } catch (e) {
            console.error("AI Error:", e);
            addToast("Terjadi kesalahan saat memanggil AI.", "error");
        } finally {
                        ...
                        <div className="p-1 space-y-4 pb-4">
                            ...
                            <div className="flex flex-col gap-1.5 mt-2">
                                ...
                                {userProfile?.role === 'super_admin' && (
                                    <div className="flex gap-2 mb-1 p-2 bg-indigo-50/50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-md">
                                        <Input 
                                            placeholder="Instruksi AI (misal: Buatkan panduan penggunaan aplikasi)..." 
                                            value={aiPrompt} 
                                        ...
                                        />
                                        <Button type="button" onClick={handleGenerateAI} disabled={isAiLoading || !aiPrompt} className="bg-indigo-600 hover:bg-indigo-700 text-white h-9 px-3 text-sm">
                                            {isAiLoading ? <Loader2 size={14} className="animate-spin" /> : <><Sparkles size={14} className="mr-1.5" /> AI Draft</>}
                                        </Button>
                                    </div>
                                )}
                                <div data-color-mode="light" className="border rounded-md overflow-hidden z-10 relative mt-1">
                                    <MDEditor 
                                        id="isi"
                                        value={formData.isi} 
                                        onChange={(val) => setFormData(prev => prev ? { ...prev, isi: val || '' } : null)} 
                                        preview="edit" 
                                        height={300} 
                                        className="w-full"
                                    />
...
export default function PengumumanPage() {
    ...
    const handleMarkAsRead = useCallback(async (id: string, readBy: string[] = []) => {
        if (!userProfile?.uid) return;
        if (readBy.includes(userProfile.uid)) return;
        ...
        try {
            ...
            // Update local state agar re-render
            setPengumumanList(prev => prev.map(p => {
                if (p.id === id) {
                    return { ...p, readBy: [...(p.readBy || []), userProfile.uid] };
                }
                return p;
            }));
        } catch (error) {
            console.error("Gagal update readBy:", error);
        }
    }, [userProfile?.uid]);
