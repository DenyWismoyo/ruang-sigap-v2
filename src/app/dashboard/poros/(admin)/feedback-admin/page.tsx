...
export default function FeedbackAdminPage() {
  ...
  if (userProfile?.role !== 'super_admin') {
    return <div className="text-center p-8 text-red-600 bg-red-100/50 backdrop-blur-md rounded-xl border border-red-200">Akses ditolak. Halaman ini hanya untuk Super Admin.</div>;
  }
    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
        <h1 className="text-3xl font-bold text-foreground flex items-center">
          <Inbox size={28} className="mr-3 text-[var(--nk-teal-mid)]" />
          Dashboard Feedback & Survei
        </h1>
        <Button
          onClick={handleExportExcel}
          disabled={isExporting}
          className="bg-[var(--nk-teal-mid)] hover:bg-[var(--nk-teal-mid)]/90 text-white"
        >
          {isExporting ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="nk-card p-4"><canvas ref={chartKepuasanRef}></canvas></div>
        <div className="nk-card p-4"><canvas ref={chartKemudahanRef}></canvas></div>
        <div className="nk-card p-4"><canvas ref={chartTipeRef}></canvas></div>
      </div>

      <div className="nk-card overflow-hidden">
        <h2 className="text-xl font-semibold text-foreground p-4 border-b border-white/10 dark:border-white/5">
        ...
        <div className="nk-table-wrapper overflow-x-auto">
          ...
          <Table className="min-w-[1000px]">
            ...
            <TableBody>
              {feedbackList.map(fb => (
                <TableRow key={fb.id}>
                  <TableCell className="whitespace-nowrap text-muted-foreground">{fb.createdAt.toDate().toLocaleDateString('id-ID', { day: '2-digit', month: 'short' })}</TableCell>
                  <TableCell className="font-medium text-foreground">{fb.userNama}<br/><span className="text-xs text-muted-foreground font-normal">{fb.userJabatan}</span></TableCell>
                  ...
                  <TableCell className="min-w-[250px] max-w-xs truncate text-foreground" title={fb.deskripsi}>{fb.deskripsi}</TableCell>
                  <TableCell className="whitespace-nowrap text-muted-foreground" title={fb.halamanTerkait}>{fb.halamanTerkait || '-'}</TableCell>
                  <TableCell>
