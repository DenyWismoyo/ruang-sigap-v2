...
import AgendaTimeline from '@/app/dashboard/natakarya/(main)/ruang-kerja/components/AgendaTimeline'; 
import VisionaryFeed from '@/app/dashboard/natakarya/(main)/ruang-kerja/components/VisionaryFeed'; 
import RuangKerjaSkeleton from '@/app/dashboard/natakarya/components/skeletons/RuangKerjaSkeleton'; 
import QuickAddTask from '@/app/dashboard/natakarya/(main)/ruang-kerja/components/QuickAddTask';
...
const JadwalDetailModal = dynamic(() => import('@/app/dashboard/natakarya/(main)/jadwal/components/JadwalDetailModal'), { ssr: false });
const QuickEditTaskModal = dynamic(() => import('@/app/dashboard/natakarya/(main)/ruang-kerja/components/QuickEditTaskModal'), { ssr: false }); 
const RuangKerjaTutorialModal = dynamic(() => import('@/app/dashboard/natakarya/(main)/ruang-kerja/components/RuangKerjaTutorialModal'), { ssr: false });
const NotulensiFormModal = dynamic(() => import('@/app/dashboard/natakarya/(fungsional)/notulensi/components/NotulensiFormModal'), { ssr: false });
const ConfirmModal = dynamic(() => import('@/app/dashboard/natakarya/components/ConfirmModal'), { ssr: false });

import { useUserAuth } from '@/context/AuthContext'; 
import { useToast } from '@/context/ToastContext'; 
import { useTheme } from '@/context/ThemeContext';
import { useRuangKerjaFeed } from '@/app/dashboard/natakarya/hooks/useRuangKerjaFeed'; 
import { useMasterData } from '@/app/dashboard/natakarya/hooks/useMasterData';         
import { useInstruksiTemplat } from '@/app/dashboard/natakarya/hooks/useInstruksiTemplat'; 
import { useJadwalActions } from '@/app/dashboard/natakarya/hooks/useJadwalActions';
...
export default function RuangKerjaPage() {
  ...
  const { data: agendaData, isLoading: isAgendaLoading1 } = useQuery({
      queryKey: ['agenda', userProfile?.uid, isPimpinan, isAdminOrTU],
      queryFn: async () => {
          if (!userProfile?.opdId || !userProfile?.uid) return { undangan: [], disposisi: [] };
          ...
          if (agendaSuratIds.length > 0) {
              ...
              allDisposisiDocs = disposisiSnapshots.flatMap(snap => snap.docs.map(d => ({ id: d.id, ...d.data() })));
          }
          
          return { undangan: agendaList, disposisi: allDisposisiDocs };
      },
      enabled: !!userProfile?.uid && !authLoading,
      staleTime: 1000 * 60 * 5, // 5 menit cache
  });
  
  const { data: jadwalInternalList = [], isLoading: isAgendaLoading2 } = useQuery({
      queryKey: ['jadwalInternal', userProfile?.uid],
      queryFn: async () => {
  ...
  const suratUndanganList = agendaData?.undangan || [];
