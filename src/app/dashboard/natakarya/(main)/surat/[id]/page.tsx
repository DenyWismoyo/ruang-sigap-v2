...
// --- HOOKS SSOT ---
import { useSuratActions } from '@/app/dashboard/natakarya/hooks/useSuratActions'; 
import { useSuratDetail } from '@/app/dashboard/natakarya/hooks/useSuratDetail';
...
import CachedPdfViewer from '@/app/dashboard/natakarya/(main)/surat/[id]/components/CachedPdfViewer';
...
const FormDisposisi = dynamic(() => import('@/app/dashboard/natakarya/(main)/surat/[id]/components/FormDisposisi'), { 
    ssr: false, 
    loading: () => <div className="p-4 text-center text-sm text-muted-foreground animate-pulse">Menyiapkan form disposisi...</div> 
});
const RiwayatDisposisi = dynamic(() => import('@/app/dashboard/natakarya/(main)/surat/[id]/components/RiwayatDisposisi'), { 
    ssr: false, 
    loading: () => <div className="p-4 text-center text-sm text-muted-foreground animate-pulse">Memuat riwayat disposisi...</div> 
});
const TindakLanjutSection = dynamic(() => import('@/app/dashboard/natakarya/(main)/surat/[id]/components/TindakLanjutSection'), { 
    ssr: false, 
    loading: () => <div className="p-4 text-center text-sm text-muted-foreground animate-pulse">Memuat form tindak lanjut...</div> 
});
const ActivityLogSection = dynamic(() => import('@/app/dashboard/natakarya/(main)/surat/[id]/components/ActivityLogSection'), { ssr: false });
const PenerimaanDisposisiModal = dynamic(() => import('@/app/dashboard/natakarya/(main)/surat/[id]/components/PenerimaanDisposisiModal'), { ssr: false });
const FormTugas = dynamic(() => import('@/app/dashboard/natakarya/(main)/tugas/components/FormTugas'), { ssr: false });


// Khusus DispositionTracker menggunakan ".then" karena bukan default export
const DispositionTracker = dynamic(() => import('@/app/dashboard/natakarya/(main)/surat/[id]/components/DispositionTracker').then(mod => mod.DispositionTracker), { 
    ssr: false, 
...
export default function DetailSuratPage() {
  ...
  const handleArchiveConfirm = async () => {
       if (!surat) return;
       const success = await archiveSurat(surat, "Diarsipkan manual");
       if(success) {
           handleRefresh();
       }
  };
  ...
  const openArchiveConfirmation = () => {
      setConfirmModalState({
          isOpen: true, title: "Konfirmasi Arsip",
          message: `Anda yakin akan mengarsipkan surat "${surat?.perihal}"?`,
