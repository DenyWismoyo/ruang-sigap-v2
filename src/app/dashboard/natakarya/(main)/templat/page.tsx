// Lokasi: src/app/dashboard/natakarya/templat/page.tsx
// [MODIFIKASI]
...
import ConfirmModal from '@/app/dashboard/natakarya/components/ConfirmModal'; // path @
...
export default function ManajemenTemplatPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const snap = await getDocs(qOpd);
            setOpdTemplatList(snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as InstruksiTemplat)));
        } catch(e) {
            console.error(e);
            setError("Gagal memuat data templat OPD.");
        }

        // Query templat dibagikan
        const qShared = query(collection(db, "instruksiTemplat"), where("sharedWithOpdIds", "array-contains", userProfile.opdId));
        try {
            const snap = await getDocs(qShared);
            setSharedTemplatList(snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as InstruksiTemplat)));
        } catch(e) {
            console.error(e);
