...
export default function FormTugas({
  ...
  const [kategoriTugas, setKategoriTugas] =
    useState<Tugas["kategoriTugas"]>("Lainnya");
  ...
  const [penanggungJawab, setPenanggungJawab] = useState<UserProfile | null>(
    null,
  );
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (suratTerkait) {
        ...
        if (
          suratTerkait.klasifikasi === "Penting" ||
          suratTerkait.klasifikasi === "Segera"
        )
          setPrioritas("Tinggi");
        setIsMandiri(false);
      }
  ...
  const stafSearchResults = useMemo(() => {
    ...
    const currentTeamIds = new Set(
      [
        penanggungJawab?.jabatanId,
        ...collaborators.map((c) => c.jabatanId),
      ].filter(Boolean),
    );
    ...
    userCache.forEach((user) => {
      if (
        ...
        user.level &&
        actingJabatanProfile.level &&
        user.level > actingJabatanProfile.level && // Hanya bawahan
        user.status === "aktif" &&
        !currentTeamIds.has(user.jabatanId) &&
        (user.searchKeywords?.some((kw) => kw.startsWith(searchLower)) ||
  ...
  const atasanResults = useMemo(() => {
    ...
    userCache.forEach((user) => {
      if (
        user.opdId === actingJabatanProfile.opdId &&
        user.level &&
        user.level < myLevel && // Hanya atasan
        user.status === "aktif" &&
        (user.searchKeywords?.some((kw) => kw.startsWith(searchLower)) ||
          user.namaLengkap.toLowerCase().includes(searchLower))
      ) {
    ...
    if (isOpen && userProfile?.opdId && !initialData && !suratTerkait) {
      const fetchRecentSurat = async () => {
        try {
          ...
          setCachedSurat(
            snapshot.docs.map(
              (doc) => ({ id: doc.id, ...doc.data() }) as Surat,
            ),
          );
        } catch (error) {
          console.error("Error fetching recent surat:", error);
        }
      };
      fetchRecentSurat();
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!judulTugas || !deskripsi || !userProfile || !actingJabatanProfile) {
      setError("Harap lengkapi Judul dan Deskripsi.");
      return;
    }
    ...
    if (!pemberiTugasUser) {
      setError(
        isMandiri
          ? 'Untuk tugas mandiri, Anda wajib memilih "Laporkan Kepada".'
          : "Tidak dapat menentukan pemberi tugas.",
      );
      return;
    }
    const penanggungJawabFinal = isMandiri ? userProfile : penanggungJawab;
    if (!penanggungJawabFinal) {
      setError("Harap pilih Penanggung Jawab untuk tugas ini.");
      return;
    }
    setLoading(true);
    try {
      ...
      const pemberiTugasJabatan = isMandiri
        ? laporKepada
          ? laporKepada.jabatanId
          : null
        : actingJabatanProfile.id;
      if (!pemberiTugasJabatan) {
        throw new Error("Gagal menentukan Jabatan Pemberi Tugas.");
      }
      ...
      const newTugas: Omit<Tugas, "id"> = {
        ...
        dariJabatanNama: isMandiri
          ? laporKepada?.namaLengkap
      ...
      if (linkedSurat) {
        ...
        await logActivity(
          linkedSurat.id!,
          actorName,
          `Membuat tugas baru: "${judulTugas}"`,
        );
      }

      const allRecipients = [
        penanggungJawabFinal,
        ...(isMandiri ? [] : collaborators),
      ];
      const uniqueRecipients = Array.from(
        new Map(allRecipients.map((u) => [u.uid, u])).values(),
      );
      ...
      for (const userToNotify of uniqueRecipients) {
        ...
        if (userToNotify?.uid) {
          ...
          batch.set(notifRef, {
            ...
            message: `Anda dilibatkan dalam tugas baru: "${judulTugas}"`,
          ...
          if (userToNotify.nomorWa) {
            try {
              await sendWhatsAppNotification(
                userToNotify.nomorWa,
                "tugas_baru",
                [actorName, judulTugas],
              );
            } catch (waError) {
              console.error(
                `Failed to send WhatsApp notification to ${userToNotify.namaLengkap}:`,
                waError,
              );
            }
          }
      ...
      setError(
        `Gagal membuat tugas baru: ${err.message || "Error tidak diketahui"}`,
      );
    } finally {
      setLoading(false);
    }
        ...
        >
          <ScrollArea className="flex-1 px-4 sm:px-6">
            <div className="space-y-4 py-4">
              {error && (
                <p className="p-3 text-sm text-center text-red-700 bg-red-100 dark:bg-red-900/20 dark:text-red-300 rounded-lg border border-red-200 dark:border-red-700">
                  {error}
                </p>
              )}
                  ...
                  <div className="col-span-2 flex items-center justify-start gap-2 p-2.5 h-10 border border-border rounded-md">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setIsMandiri(checked as boolean)
                      }
                    />
                    <Label
                      htmlFor="isMandiri"
              ...
              {!initialData && (
                <div>
                  ...
                  {linkedSurat ? (
                    ...
                    <Popover
                      open={suratPopoverOpen}
                      onOpenChange={setSuratPopoverOpen}
                    >
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            if (e.target.value.length >= 2)
                              setSuratPopoverOpen(true);
                            else setSuratPopoverOpen(false);
                          }}
                        />
                      </PopoverTrigger>
              ...
              {isMandiri && (
                <div>
                  ...
                  {laporKepada ? (
                    ...
                    <Popover
                      open={atasanPopoverOpen}
                      onOpenChange={setAtasanPopoverOpen}
                    >
                      <PopoverTrigger asChild>
                        <Input
                          id="atasanSearch"
                          ...
                          onChange={(e) => {
                            ...
                            if (e.target.value.length >= 2)
                              setAtasanPopoverOpen(true);
                            else setAtasanPopoverOpen(false);
                          }}
                        />
                      </PopoverTrigger>
                      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0">
              ...
              {!isMandiri && (
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Popover
                      open={stafPopoverOpen}
                      onOpenChange={setStafPopoverOpen}
                    >
                      <PopoverTrigger asChild>
                        <Button
                          id="pj-search"
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button type="button" variant="secondary" size="sm">
                            <UserPlus size={14} className="mr-1.5" /> Tambah
                            Kolaborator
                          </Button>
                        </PopoverTrigger>
                        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <Select
                    value={kategoriTugas}
                    onValueChange={(value) => setKategoriTugas(value as any)}
                  >
                    <SelectTrigger id="kategoriTugas">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="Penyusunan Laporan">
                        Penyusunan Laporan
                      </SelectItem>
                      <SelectItem value="Analisis Data">
                        Analisis Data
                      </SelectItem>
                      <SelectItem value="Persiapan Materi">
                        Persiapan Materi
                      </SelectItem>
                ...
                <div>
                  ...
                  <Select
                    value={prioritas}
                    onValueChange={(value) => setPrioritas(value as any)}
                  >
