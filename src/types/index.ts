...
export interface RepositoryItem { 
  ...
  createdBy: string; 
  createdAt: any; 
  updatedAt?: any; // [UPDATE] Menyimpan waktu update terakhir
  ...
  sharedWithUsers?: string[]; // UID pengguna spesifik (Granular Sharing)
  isFavorite?: boolean;

  // [BARU] Soft Delete & Recycle Bin
  isDeleted?: boolean;
  deletedAt?: any; // Timestamp
  deletedBy?: string; // UID penghapus

  // [BARU] Integrasi Arsip Otomatis (Auto-Archive)
  linkedToType?: 'surat' | 'tindak_lanjut' | 'notulensi' | 'bukti_kinerja';
  linkedToId?: string;
  autoGenerated?: boolean; // True jika dibuat oleh sistem
  retentionUntil?: any; // Waktu retensi / pemusnahan
  arsipKode?: string; // Kode klasifikasi

  // [BARU] AI Smart Folder & Classification
  aiSummary?: string;
  aiClassification?: string;
}
export interface LogbookKegiatan { id: string; deskripsi: string; selesai: boolean; tugasTerkaitId?: string; tugasTerkaitJudul?: string; kategori?: 'Surat' | 'Disposisi' | 'Tugas' | 'Rapat' | 'Laporan' | 'Umum'; sumber?: 'manual' | 'copilot' | 'laporan_tindak_lanjut' | 'tugas' | 'checklist'; suratTerkaitId?: string; suratPerihal?: string; disposisiTerkaitId?: string; waktuMulai?: string; waktuSelesai?: string; createdAt?: string; aktivitasId?: number; aktivitasNama?: string; }
